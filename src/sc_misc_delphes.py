...
def calculate_cosine(pred, X):
    ...
    p4nm = vector.arr({"px": p_nu_x, "py": p_nu_y, "pz": p_nu_z, "M": np.zeros_like(p_nu_x)}).to_xyzt()

    pt_l = np.array(X['Pt_Lep'])
    eta_l = np.array(X['Eta_Lep'])
    phi_l = np.array(X['Phi_Lep'])
    M = np.array(X['M_Lep'])
    p4Mu = vector.arr({"pt": pt_l, "eta": eta_l, "phi": phi_l, "M": M}).to_xyzt()

    pt_b = np.array(X['Pt_BJ1'])
    eta_b = np.array(X['Eta_BJ1'])
    phi_b = np.array(X['Phi_BJ1'])
    M = np.array(X['M_BJ1'])
    p4j1 = vector.arr({"pt": pt_b, "eta": eta_b, "phi": phi_b, "M": M}).to_xyzt()

    pt_q = np.array(X['Pt_LJ'])
    eta_q = np.array(X['Eta_LJ'])
    phi_q = np.array(X['Phi_LJ'])
    M = np.array(X['M_LJ'])
    p4j2 = vector.arr({"pt": pt_q, "eta": eta_q, "phi": phi_q, "M": M}).to_xyzt()

    p_phi_x = np.array(pred[:, 3])
    p_phi_y = np.array(pred[:, 4])
    p_phi_z = np.array(pred[:, 5])
    M = 400 * np.ones_like(p_phi_x)
    p4phi = vector.arr({"px": p_phi_x, "py": p_phi_y, "pz": p_phi_z, "M": M}).to_xyzt()
    ...
    p3Lepton = boostedLepton.to_Vector3D()
    p3LJet = boostedLJet.to_Vector3D()
